      ...
      <div className="flex h-14 items-center justify-end gap-2">
        <div className="bg-sidebar pointer-events-auto flex h-12 items-center gap-2 rounded-md px-2.5">
          ...
          {/* <Button
            ...
            variant="default"
            onClick={() => {
              window.open("https://prajwal-38.github.io", "_blank");
            }}
          >
            Discord
          </Button> */}
...
const DownloadPopover = ({ stateManager }: { stateManager: StateManager }) => {
  ...
  const handleExport = async () => {
    ...
    if (exportType === 'json') {
      ...
      console.log('✅ Timeline exported as JSON');
      setOpen(false);
    } else if (exportType === 'mp4') {
      // Export timeline as MP4 using backend video stitching
      console.log('🎬 Starting MP4 export...');
      actions.setState({ payload: data });
      actions.startExport();
    } else {
      // For GIF, try the server-side rendering (may not work)
      console.log('🎬 Attempting server-side export for', exportType);
      actions.setState({ payload: data });
      actions.startExport();
    }
  };
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          className="flex h-10 gap-2 border-2 border-cyan-500/50 bg-gradient-to-r from-cyan-500/10 to-pink-500/10 hover:from-cyan-500/20 hover:to-pink-500/20 text-white font-semibold shadow-lg hover:shadow-cyan-500/25 transition-all duration-200"
          variant="outline"
        >
          <Download width={20} className="text-cyan-400" />
      ...
      >
        ...
        <div>
          <Button
            ...
            className="w-full h-12 bg-gradient-to-r from-cyan-500 to-pink-500 hover:from-cyan-600 hover:to-pink-600 text-white font-bold text-lg shadow-lg hover:shadow-xl transition-all duration-200"
          >
