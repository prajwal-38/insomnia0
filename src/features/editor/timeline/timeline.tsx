...
const Timeline = ({ stateManager, isAIChatOpen, onToggleAIChat }: TimelineProps) => {
  ...
  // Listen for video translation completion events
  useEffect(() => {
    const handleVideoTranslationCompleted = (event: CustomEvent) => {
      const { sceneId, translatedVideoUrl, translationInfo } = event.detail;

      console.log(`🌐 Main Timeline: Video translation completed for scene ${sceneId}`);
      console.log(`🎬 New translated video URL: ${translatedVideoUrl}`);

      // The timeline integration service should have already updated the video source
      // This is just for logging and any additional UI updates if needed
      console.log('✅ Main Timeline: Translation event received, video source should be updated automatically');
    };
